...
class BattleshipClient:
    ...
    def connect(self):
        ...
        # Отправка сообщения о готовности к началу расстановки кораблей после подключения
        ready_message = self.client_socket.recv(1024).decode()
        if ready_message == "START_SHIP_PLACEMENT":
            self.start_ship_placement()

    def start_ship_placement(self):

        app = QApplication(sys.argv)
        window = ShipPlacementWindow(player=1, game_setup_window=self)
        window.show()
        app.exec()

    def send_ship_placement(self, ship_placement_data):
        self.client_socket.sendall(ship_placement_data.encode())  # Отправка данных о расстановке кораблей на сервер
